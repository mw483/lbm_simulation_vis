...
void ParticleFootPrint::allocate_footprint (Setting& setting) {
	...
	footprint = new int[n_sensor*y_domain*x_domain];
	...
	for (int i=0; i<n_sensor; i++) {
		for (int j=0; j<y_domain; j++) {
			for (int k=0; k<x_domain; k++) {
				footprint[k+j*x_domain+i*x_domain*y_domain] = 0;
			}
		}
	}
}
...
void ParticleFootPrint::cal_footprint (float x, float y, float z, int index, Setting& setting) {
	...
	for (int i=0; i<n_sensor; i++) {
		...
		if (xs_min <= x && x <= xs_max && ys_min <= y && y <= ys_max && zs_min <= z && z <= zs_max ) {
			...
			float source_x 		= p_source[source_index].pos_x;
			float source_y		= p_source[source_index].pos_y;

			int	x_s	= (int)(source_x/dx);
			int y_s	= (int)(source_y/dx);

			footprint[x_s + y_s*x_domain + i*y_domain*x_domain]++;
			num_p_sensor[i]++;
		}
	}
}
...
void ParticleFootPrint::output_footprint (Setting& setting) {
	...
	for (int i=0; i<n_sensor; i++) {
		...
		for (int j=0; j<y_domain; j++) {
			for (int k=0; k<x_domain; k++) {
				fFile <<  footprint[k+j*x_domain+i*y_domain*x_domain];
